...
contract UnitERC721CollectionTest is Test {
    ...
    function test_OwnerMint_Success() public {
        ...
        vm.startPrank(setup.owner);
        vm.expectEmit(true, true, true, true);
        emit Minted(setup.owner, 1, 1);
        setup.collection.ownerMint(setup.owner, 1);
        vm.stopPrank();

        assertEq(setup.collection.ownerOf(1), setup.owner);
        assertEq(setup.collection.getTotalMinted(), 1);
    }
    ...
    function test_OwnerMint_BatchSuccess() public {
        ...
        for (uint256 i = 1; i <= 5; i++) {
            assertEq(setup.collection.ownerOf(i), setup.owner);
        }
    ...
    function test_OwnerMint_RespectsMaxSupply() public {
        ...
        ERC721Collection smallCollection = new ERC721Collection(smallParams);
